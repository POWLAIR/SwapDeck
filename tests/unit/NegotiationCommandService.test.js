...
// $transaction reçoit un callback (tx) → on lui passe mockDb lui-même comme tx
...
describe('refuseTrade', () => {
  ...
  it('lève NOT_FOUND si la trade est introuvable', async () => {
    prisma.trade.findUnique.mockResolvedValue(null);
    await expect(service.refuseTrade(999, 2)).rejects.toThrow('NOT_FOUND');
  });

  it('lève INVALID_STATUS si Prisma retourne P2025 (concurrent refuse)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade());
    prisma.trade.update.mockRejectedValue(Object.assign(new Error('P2025'), { code: 'P2025' }));
    await expect(service.refuseTrade(1, 2)).rejects.toThrow('INVALID_STATUS');
  });
...
describe('counterTrade', () => {
  ...
  it('lève INVALID_STATUS si Prisma retourne P2025 dans la transaction (concurrent counter)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade());
    prisma.trade.update.mockRejectedValue(Object.assign(new Error('P2025'), { code: 'P2025' }));
    await expect(service.counterTrade(1, 2, [], [], 'Contre-offre')).rejects.toThrow('INVALID_STATUS');
  });
});
...
describe('cancelTrade', () => {
  ...
  it('rejette si la trade est déjà clôturée (INVALID_STATUS)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade({ status: 'ACCEPTED' }));
    await expect(service.cancelTrade(1, 1)).rejects.toThrow('INVALID_STATUS');
  });

  it('lève NOT_FOUND si la trade est introuvable', async () => {
    prisma.trade.findUnique.mockResolvedValue(null);
    await expect(service.cancelTrade(999, 1)).rejects.toThrow('NOT_FOUND');
  });

  it('lève INVALID_STATUS si Prisma retourne P2025 (concurrent cancel)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade());
    prisma.trade.update.mockRejectedValue(Object.assign(new Error('P2025'), { code: 'P2025' }));
    await expect(service.cancelTrade(1, 1)).rejects.toThrow('INVALID_STATUS');
  });
...
describe('addMessage', () => {
  ...
  it('accepte un message sur une trade COUNTERED (toujours ouverte)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade({ status: 'COUNTERED' }));
    prisma.message.create.mockResolvedValue({ id: 2, body: 'Info complémentaire', authorId: 1 });

    const result = await service.addMessage(1, 1, 'Info complémentaire');
    expect(result.body).toBe('Info complémentaire');
  });

  it('rejette si la trade est clôturée (ACCEPTED)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade({ status: 'ACCEPTED' }));
    await expect(service.addMessage(1, 1, 'Bonjour')).rejects.toThrow('INVALID_STATUS');
  });

  it('rejette si la trade est clôturée (REFUSED)', async () => {
    prisma.trade.findUnique.mockResolvedValue(makeTrade({ status: 'REFUSED' }));
    await expect(service.addMessage(1, 1, 'Bonjour')).rejects.toThrow('INVALID_STATUS');
  });
});
