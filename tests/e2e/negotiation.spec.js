...
test.describe('Négociation — cas usuels', () => {
  ...
  test('Mira peut faire une contre-proposition', async ({ page }) => {
    ...
    // Redirigé vers la NOUVELLE trade (ID différent de la trade originale)
    // On utilise un prédicat car la page est déjà sur /#trade/${tradeId} — la
    // regex seule résoudrait immédiatement sans attendre la vraie navigation.
    await page.waitForURL(
      (url) => {
        const m = url.hash.match(/#trade\/(\d+)/);
        return m !== null && m[1] !== String(tradeId);
      },
      { timeout: 10000 }
    );
...
test.describe('Négociation — cas d\'erreur', () => {
  test('impossible de proposer sans sélectionner de cartes', async ({ page }) => {
    ...
    // Le bouton "Proposer un échange" pré-sélectionne la carte ciblée dans le
    // formulaire. On décoche toutes les cases pour tester la validation.
    for (const cb of await page.locator('input[type="checkbox"]:checked').all()) {
      await cb.uncheck();
    }
    ...
    // Le formulaire utilise des erreurs inline (pas window.alert)
    await expect(page.locator('#form-error')).toContainText(/carte/i);
